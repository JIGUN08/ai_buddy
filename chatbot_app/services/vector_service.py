...
def _get_memory_contexts(user, user_message_text):
    ...
    try:
        ...
        # vector_service.query_similar_messages는 딕셔너리를 반환합니다. 'documents' 키의 목록을 가져와야 합니다.
        retrieved_results = vector_service.query_similar_messages(pinecone_index, user_message_text, user.id, n_results=5)
        retrieved_docs = retrieved_results.get("documents", []) # 리스트를 추출하도록 수정
